...
public class Crypto
{
    ...
    public static bool DecryptString()
    {
        ...
        {
            _exception = new CryptographicException(ex.Message, ex.InnerException);
            // MsgBox("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=" & vbCr & vbCr & ex.Message)
            return false;
        }
    ...
    public static bool GenerateHash(string Content)
    {
        ...
        HashAlgorithm hashAlgorithm = null/* TODO Change to default(_) if this is not a reference type */;
    ...
    {
        string key_convert = Convert.ToBase64String(Content);
    ...
    private static byte[] SymmetricEncrypt(SymmetricAlgorithm Provider, byte[] plainText, string key, int keySize)
    {
        ...
        byte[] ivBytes = null;
        switch (keySize / (double)8) // Determine which initialization vector to use
        {
            case 8:
                {
                    ivBytes = IV_8;
        ...
        byte[] keyStream = DerivePassword(key, (int)(keySize / (double)8));
    ...
    private static byte[] RSAEncrypt(byte[] plainText)
    {
        ...
        int blockCount = (int)Math.Floor(plainText.Length / (double)RSA_BLOCKSIZE);
        ...
        RSACryptoServiceProvider Provider = new RSACryptoServiceProvider((int)KeySize.RSA);
        ...
        for (int blockIndex = 0; blockIndex <= blockCount - 1; blockIndex++)
        {
            ...
            byte[] currentBlock = new byte[thisBlockLength - 1 + 1];
    ...
    private static byte[] RSADecrypt(string encText)
    {
        ...
        for (int blockIndex = 0; blockIndex <= blockCount - 1; blockIndex++)
        {
            ...
            var currentBlockHex = encText.Substring(blockIndex * (RSA_DECRYPTBLOCKSIZE * 2), RSA_DECRYPTBLOCKSIZE * 2);
    ...
    private static byte[] HexToBytes(string Hex)
    {
        ...
        for (int n = 0; n <= numBytes - 1; n++)
        {
            ...
            bytes[n] = (byte)int.Parse(hexByte, NumberStyles.HexNumber);
        }
        return bytes;
    }
