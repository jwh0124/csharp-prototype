...
namespace SupremaSDK.Libs
{
    public class Util
    {
        public static T BytesToStruct<T>(ref byte[] source)
        {
            ...
            nint buffer = Marshal.AllocHGlobal(structSize);
        ...
        public void CopyMemory<T>(ref nint source, ref nint target)
        {
            Type structType = typeof(T);
            int structSize = Marshal.SizeOf(structType);
        ...
        public delegate int FUNC_BS2_GetAll(nint context, uint deviceId, out nint obj, out uint numItem);
        public static BS2ErrorCode CSP_BS2_GetAll<CSP_T, CXX_T>(nint context, uint deviceId, out CSP_T[] ItemsObj, out uint numItem, FUNC_BS2_GetAll func)
        {
            nint _itemsObj = nint.Zero;
            uint _itemNum = 0;
        ...
        public delegate int FUNC_BS2_GetItems(nint context, uint deviceId, nint Ids, uint IdCount, out nint obj, out uint numItem);
        public static BS2ErrorCode CSP_BS2_GetItems<CSP_ID_T, CSP_T, CXX_ID_T, CXX_T>(nint context, uint deviceId, CSP_ID_T[] Ids, uint idCount, out CSP_T[] ItemsObj, out uint numItem, FUNC_BS2_GetItems func)
        {
            Translator<CSP_ID_T, CXX_ID_T> transID = new Translator<CSP_ID_T, CXX_ID_T>();
            CXX_ID_T Id = AllocateStructure<CXX_ID_T>();

            int IdSize = Marshal.SizeOf(typeof(CXX_ID_T));
            nint _IDObj = Marshal.AllocHGlobal(IdSize * (int)idCount);
            nint _curIDObj = _IDObj;
        ...
        public delegate int FUNC_BS2_RemoveItems(nint context, uint deviceId, nint Ids, uint IdCount);
        public static BS2ErrorCode CSP_BS2_RemoveItems<CSP_ID_T, CXX_ID_T>(nint context, uint deviceId, CSP_ID_T[] Ids, uint idCount, FUNC_BS2_RemoveItems func)
        {
            Translator<CSP_ID_T, CXX_ID_T> transID = new Translator<CSP_ID_T, CXX_ID_T>();
            CXX_ID_T Id = AllocateStructure<CXX_ID_T>();
        ...
        public delegate int FUNC_BS2_SetItems(nint context, uint deviceId, nint ItemsObj, uint ItemCount);
        public static BS2ErrorCode CSP_BS2_SetItems<CSP_T, CXX_T>(nint context, uint deviceId, CSP_T[] ItemsObj, uint ItemCount, FUNC_BS2_SetItems func)
        {
            Translator<CSP_T, CXX_T> transItem = new Translator<CSP_T, CXX_T>();
            CXX_T Item = AllocateStructure<CXX_T>();

            int ItemSize = Marshal.SizeOf(typeof(CXX_T));
            nint _ItemsObj = Marshal.AllocHGlobal(ItemSize * (int)ItemCount);
            nint _curItemObj = _ItemsObj;
        ...
        public static T[] AllocateStructureArray<T>(int count)
        {
            ...
            Array.Clear(empty, 0, empty.Length);
            nint buffer = Marshal.AllocHGlobal(structSize * count);
            nint curBuffer = buffer;
            Marshal.Copy(empty, 0, buffer, structSize * count);
            for (int idx = 0; idx < count; idx++)
            {
                result[idx] = (T)Marshal.PtrToStructure(curBuffer, typeof(T));
                curBuffer = (nint)((long)curBuffer + structSize);
            }
        ...
        public static T ConvertTo<T>(byte[] src)
        {
            ...
            nint buffer = Marshal.AllocHGlobal(src.Length);
        ...
        public static bool GetInput(out int input)
        {
            ...
            {
                return int.TryParse(inputStr, out input);
            }
            else
            {
                input = 0;
        ...
        public static ushort GetInput(ushort defaultValue)
        {
            string inputStr = Console.ReadLine();
            if (inputStr.Length > 0)
            {
        ...
        public static uint GetInput(uint defaultValue)
        {
            string inputStr = Console.ReadLine();
            if (inputStr.Length > 0)
            {
        ...
        public static bool GetTimestamp(string formatString, uint defaultValue, out uint timestamp)
        {
            string inputStr = Console.ReadLine();

            if (defaultValue == 0)
            {
                defaultValue = Convert.ToUInt32(ConvertToUnixTimestamp(DateTime.Now));
            }
        ...
        public static bool GetTime(string formatString, uint defaultValue, out uint time)
        {
            string inputStr = Console.ReadLine();
            time = 0;
            if (inputStr.Length > 0)
            {
                ...
                {
                    time = (uint)(dateTime.Hour * 60 * 60 + dateTime.Minute * 60 + dateTime.Second);
                }
            }
        ...
        public static bool LoadBinary(string filePath, out nint binaryData, out uint binaryDataLen)
        {
            bool handled = false;
            FileStream fs = null;

            binaryData = nint.Zero;
            binaryDataLen = 0;
            try
            {
        ...
        public static string getActionMsg(BS2Action action)
        {
            ...
            switch (actionType)
            {
                ...
                case BS2ActionTypeEnum.RELAY:
                    {
                        BS2RelayAction relay = ConvertTo<BS2RelayAction>(action.actionUnion);
                ...
                case BS2ActionTypeEnum.TTL:
                    {
                        BS2OutputPortAction outputPort = ConvertTo<BS2OutputPortAction>(action.actionUnion);
                ...
                case BS2ActionTypeEnum.DISPLAY:
                    {
                        BS2DisplayAction display = ConvertTo<BS2DisplayAction>(action.actionUnion);
                ...
                case BS2ActionTypeEnum.SOUND:
                    {
                        BS2SoundAction sound = ConvertTo<BS2SoundAction>(action.actionUnion);
                        return string.Format("SoundAction soundIndex[{0}] count[{1}]", sound.soundIndex, sound.count);
                    }
                case BS2ActionTypeEnum.LED:
                    {
                        BS2LedAction led = ConvertTo<BS2LedAction>(action.actionUnion);
                        string ledSignalStr = "";
                ...
                case BS2ActionTypeEnum.BUZZER:
                    {
                        BS2BuzzerAction buzzer = ConvertTo<BS2BuzzerAction>(action.actionUnion);
                ...
                case BS2ActionTypeEnum.LIFT:
                    {
                        BS2LiftAction lift = ConvertTo<BS2LiftAction>(action.actionUnion);
        ...
        {
            DateTime eventTime = ConvertFromUnixTimestamp(eventLog.dateTime);
            string userID = Encoding.ASCII.GetString(eventLog.userID).TrimEnd('\0');
            if (userID.Length == 0)
            {
                userID = "unknown";
            }
        ...
        public static uint GetInputMasterOrSlaveID(uint masterID)
        {
            uint id = 0;
        ...
        public static uint GetInputSlaveID()
        {
            uint id = 0;
        ...
        public static extern void CopyMemory(nint dest, nint src, uint count);
    }
}
